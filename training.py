...
from models import build_discriminator, build_generator, build_encoder, discriminator
from utils import plot_ts

def train(models, SHAPE, data, params):
...
def build_and_train_models(seq_length, SHAPE, data,train_steps=2000):
    """Load the dataset, build MTSS discriminator,
    generator, and adversarial models.
    Call the MTSS train routine.
    """

    dataX, _, _ = data_loading(seq_length, data)
    ...
    dis0 = build_discriminator(inputs, SHAPE, z_dim=z_dim)
    ...
    dis1 = discriminator(inputs, SHAPE = SHAPE, num_codes=z_dim)

    # loss fuctions:
    # 1) probability time series arrays is real (adversarial1 loss)
    ...
    gen0, gen1 = build_generator(latent_codes, SHAPE= SHAPE)
    ...
    enc0, enc1 = build_encoder((inputs, feature0), SHAPE= SHAPE, num_labels = num_labels)
    ...
    # build adversarial0 model = generator0 + discriminator0 + encoder1
    ...
    # loss functions:
    # 1) prob labels are real (adversarial1 loss)
    ...
    # build adversarial1 model = generator1 + discriminator1 + encoder0
    optimizer = RMSprop(lr=lr * 0.5, decay=decay * 0.5)
    enc0.trainable = False   # encoder1 weights frozen
    dis1.trainable = False   # discriminator1 weights frozen
    ...
    # loss functions:
    # 1) prob feature1 is real (adversarial0 loss)
    ...
    gen0, gen1 = train(models = models, SHAPE= SHAPE, data= data, params=params)
